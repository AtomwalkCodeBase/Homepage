...
import { FaChartBar, FaCheck, FaHashtag } from 'react-icons/fa';
...
// console.log("project.teamMembers", project.teamMembers)
  ...
  return project.teamMembers.map((emp) => {
    ...
    //   (sum, d) => sum + Number(d.effort),
    //   0
    // );

    const totalEffort = emp.effort
    ...
    return {
      ...
      totalEffort,
      workDays,
      status: emp.activity_status,
      planned_start_date: emp.planned_start_date,
      planned_end_date: emp.planned_end_date,
      actual_start_date: emp.actual_start_date,
      actual_end_date: emp.actual_end_date,
      rawProject: project,
    };
  });
...
// console.log("employeeWorkLog",employeeWorkLogs)
                ...
                <div style={{ height: "600px", overflowY: "auto" }}>
                  {employeeWorkLogs.map((member, index) => (
                    ...
                    <TeamMember key={index}>
                      ...
                      {/* <Badge variant={getStatusVariant(project.project_period_status)}>{project.project_period_status}</Badge> */}
                      <MemberHours>{member.totalEffort} hrs</MemberHours>
                    </TeamMember>
                    <TeamMember style={{paddingTop: 0}}>
                      <MemberHours>Planned date:</MemberHours> <MemberRole style={{fontSize: 14}}> {member.planned_start_date || "--"} to {member.planned_end_date || "--"}</MemberRole>
                    </TeamMember>
                    </Divider>
                  ))}
                </div>
        ...
        workLogs.map((employee) => (
          <EmployeeWorkCard key={employee.id}>
            ...
            <EmployeeHeader>
              ...
              <div>
                <Badge variant={getStatusVariant(employee.status ? "Complete" : employee.rawProject.project_period_status )}>
              ...
              <TeamMember  style={{paddingTop: 0, paddingLeft: 20}}>
                ...
                <MemberHours>End Date:</MemberHours> <MemberRole style={{fontSize: 14}}> {employee.actual_end_date || "Activity not Ended"}</MemberRole>
                 <Button variant="primary" size="sm" title="View Employee's Activity" style={{marginLeft: "auto"}} onClick={()=> Navigate(`/timesheet?empid=${employee.id}&&name=${employee.name}&&path=modal`) }>
                        <FaChartBar /> View Employee's Activity
                      </Button>
              </TeamMember>
              </CardTop>
                ...
                {employee.workDays.map((day, index) => (
                  <WorkDayItem key={index}>
                    <WorkDayHeader>
                      ...
                      {/* <Badge variant={getStatusVariant(day.status)}>{day.status}</Badge> */}
