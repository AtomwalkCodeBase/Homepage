...
const ProjectManagementAddForm = ({ isOpen, onClose, activity, onSubmit, onActivitySubmit, modalContext, forceMode }) => {
  ...
  const isTodayPlannedEnd = activity.planned_end_date === todayDayLogKey;
  ...
  const isCompleteAction = modalContext?.type === "complete";
  ...
  const isRemarksRequired =
    (isContinueAction && isTodayPlannedEnd) ||     // Rule 1
    (isUpdateMode && isLessThanAssigned);          // Rule 2

  const hasRemarks = formData.remarks.trim().length > 0;

  const isDisabled =
    !formData.endTime ||
    (!isZeroItems && !formData.noOfItems) ||       // Hide field but allow 0
    (isRemarksRequired && !hasRemarks);
    ...
    <ModalOverlay onClick={onClose}>
     ...
     {/* <Modal onClose={onClose}> */}
        ...
        <ModalBody>
          ...
          {isUpdateMode &&
            <FormGroup>
              <Label><FiFileText /> Remarks {isRemarksRequired ? "*" : "(Optional)"}</Label>
              <TextArea
          ...
          <FormGroup>
            ...
            <FileUploadContainer onClick={() => document.getElementById("file-upload").click()}>
              <FileInput id="file-upload" name="file" type="file" onChange={handleFileChange}   accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.ppt,.pptx," />
              <FileUploadIcon>
                <FaUpload />
              </FileUploadIcon>
